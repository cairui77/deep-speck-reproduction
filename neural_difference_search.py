...
def parse_args():
  parser = argparse.ArgumentParser(description='Few-shot neural input difference search for Speck32/64.')
  parser.add_argument('--pretrain-samples', type=int, default=10**7, help='Training samples for 3-round preprocessor.')
  parser.add_argument('--pretrain-rounds', type=int, default=3, help='Rounds for preprocessor training.')
  parser.add_argument('--pretrain-epochs', type=int, default=1, help='Epochs for preprocessor training.')
  parser.add_argument('--runs', type=int, default=10, help='Number of optimizer restarts.')
  parser.add_argument('--search-steps', type=int, default=2000, help='Optimizer steps per run.')
  return parser.parse_args()

def main():
  args = parse_args();
  net_pp = train_preprocessor(args.pretrain_samples, args.pretrain_rounds, args.pretrain_epochs);
  for i in range(args.runs):
    print("Run ",i,": ");
    diff, val_acc = greedy_optimizer_with_exploration(
      randint(0,2**32-1),
      lambda x: evaluate_diff(x, net_pp, args.pretrain_rounds),
      n=args.search_steps
    );
    extend_attack(diff, net_pp, args.pretrain_rounds, val_acc);
