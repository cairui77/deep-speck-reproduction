...
def train_speck_distinguisher(num_epochs, num_rounds=7, depth=1):
    ...
    val_acc_hist = h.history.get('val_accuracy', h.history.get('val_acc'));
    if val_acc_hist is not None:
      np.save(wdir+'h'+str(num_rounds)+'r_depth'+str(depth)+'_val_acc.npy', val_acc_hist);
    np.save(wdir+'h'+str(num_rounds)+'r_depth'+str(depth)+'_val_loss.npy', h.history['val_loss']);
    dump(h.history,open(wdir+'hist'+str(num_rounds)+'r_depth'+str(depth)+'.p','wb'));
    if val_acc_hist is not None:
      print("Best validation accuracy: ", np.max(val_acc_hist));
    else:
      print("Validation accuracy key not found in history. Available keys:", list(h.history.keys()));
    return(net, h);
