...
def parse_args():
  parser = argparse.ArgumentParser(description='Proof-of-concept key recovery for round-reduced Speck32/64.')
  parser.add_argument('--runs-11r', type=int, default=100, help='Number of 11-round attack runs.')
  parser.add_argument('--runs-12r', type=int, default=20, help='Number of 12-round attack runs.')
  parser.add_argument('--skip-12r', action='store_true', help='Skip the expensive 12-round runs.')
  parser.add_argument('--quick', action='store_true', help='Quick smoke mode: 2 runs for 11 rounds, no 12-round run.')
  return parser.parse_args()

def main():
  args = parse_args();

  runs_11r = args.runs_11r;
  runs_12r = args.runs_12r;
  skip_12r = args.skip_12r;
  if args.quick:
    runs_11r = 2;
    skip_12r = True;

  arr1, arr2, good = test(runs_11r);
  np.save(open('run_sols1.npy','wb'),arr1);
  np.save(open('run_sols2.npy','wb'),arr2);
  np.save(open('run_good.npy','wb'),good);

  if not skip_12r and runs_12r > 0:
    arr1, arr2, good = test(
      runs_12r,
      nr=12,
      num_structures=500,
